...
aug = ImageDataGenerator(rotation_range=30, zoom_range=0.15,
	...
	brightness_range = (1.2,1.5),horizontal_flip=True, fill_mode="nearest")
#
...
model.reset_states()
opt = SGD(lr=0.04, decay =config.DECAY,nesterov=True)
...
model.save("./intermediary-pt5.model", overwrite=True)
###########
testGen = HDF5DatasetGenerator(config.TEST_HDF5, config.BATCH_SIZE,
...
print("[INFO] serializing model...")
model.save("morning_tune-pt5.model", overwrite=True)
